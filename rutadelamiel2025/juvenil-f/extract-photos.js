...
const ALBUM_URL = "https://photos.google.com/share/AF1QipPYW22GJxA-htL39UJ5KaMsao-LPSQt94PWD0b5dMqMY4d3dnYl01LKsXwQDbjugQ?key=c25SMi1KaEFaQTE1d0MzREpVRTAzU3R5TEFJZWVB"
const OUTPUT_PATH = path.resolve('fotos.json');
const TARGET_WIDTH = 5184;
const TARGET_HEIGHT = 3456;
// **NEW CONSTANTS FOR RESIZED IMAGE URL**
const WIDTH_RESIZED = 640;
const HEIGHT_RESIZED = 480;
...
// **New check for resized dimensions**
if (isNaN(WIDTH_RESIZED) || isNaN(HEIGHT_RESIZED) || WIDTH_RESIZED <= 0 || HEIGHT_RESIZED <= 0) {
    console.error('ERROR: WIDTH_RESIZED and HEIGHT_RESIZED must be positive integers.');
    process.exit(1);
}
...
(async () => {
    ...
    async function extractCurrentItems() {
        // Pass target size parameters AND resized size parameters to the page evaluation context
        const batch = await page.evaluate((targetW, targetH, resizedW, resizedH) => { // **Added resizedW, resizedH**
            ...
            const fixBg = (style, width, height) => { // **Modified to accept width and height**
                if (!style) return null;
                const m = style.match(/url\(["']?(.*?)["']?\)/);
                const rawUrl = m ? m[1] : null;
                return rawUrl ? resizeUrl(rawUrl, width, height) : null;
            };

            return Array.from(document.querySelectorAll('.p137Zd')).map(el => {
                const bgEl = el.querySelector('.RY3tic');
                // **Generate both original and resized URLs**
                const bgUrl = bgEl ? fixBg(bgEl.style.backgroundImage, targetW, targetH) : null;
                const bgUrlResized = bgEl ? fixBg(bgEl.style.backgroundImage, resizedW, resizedH) : null;
                ...
                return {
                    ...
                    date: pDate,
                    imageUrl: bgUrl,
                    imageUrlResized: bgUrlResized // **New property added**
                };
            });
        }, TARGET_WIDTH, TARGET_HEIGHT, WIDTH_RESIZED, HEIGHT_RESIZED); // **Pass new variables here**

        let newCount = 0;
        for (const item of batch) {
            const href = normalizeHref(item.href);
            if (href && item.date && !seen.has(href)) {
                // **Store imageUrlResized in the seen Map**
                seen.set(href, { href, date: item.date, 'imageurl': item.imageUrl, 'imageurlresized': item.imageUrlResized });
                newCount++;
            }
        }
        return newCount;
